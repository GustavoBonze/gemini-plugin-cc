...
- Leave model unset by default. Add `--model` only when the user explicitly asks for one. Accepted aliases: `flash` → `gemini-2.5-flash` (fast), `pro` → `gemini-3.1-pro-preview` (powerful/default), `thinking` → 2.5 Flash with reasoning tokens.
- Leave `--depth` unset unless the user's request implies shallow or deep analysis. Accepted values: `low` (quick scan, flash model), `medium` (default), `high` (deep analysis, pro model + expanded prompt).
