...
- Leave model unset by default. Only add `--model` when the user explicitly asks for a specific model. Accepted aliases: `flash` (fast), `pro` (powerful, default).
- Leave `--depth` unset unless the user explicitly asks for faster/deeper analysis. Accepted values: `low` (fast scan), `medium` (default), `high` (deep analysis).
...
- If the user is clearly asking to continue prior Gemini work such as "continue", "keep going", "resume", "apply the top fix", or "dig deeper", add `--resume-last` unless `--fresh` is present.
